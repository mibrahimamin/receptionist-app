...
export async function POST(request: NextRequest) {
  ...
  if (!serviceId || !startTime || !name || (!email && !phone)) {
  ...
  const dateISO = format(toZonedTime(parseISO(startTime), business.timezone), "yyyy-MM-dd");
  const { startUtc, endUtc } = dayRangeUtc(dateISO, business.timezone);
  ...
  const [{ data: hours }, { data: existing }] = await Promise.all([
    supabaseServer.from("business_hours").select("*").eq("business_id", business.id),
    supabaseServer
      .from("appointments")
  ...
  // stale client-side slot list (e.g. two visitors booking at once).
  ...
  if (apptError || !appointment) {
    return NextResponse.json({ error: "Could not book that appointment." }, { status: 500 });
  }
