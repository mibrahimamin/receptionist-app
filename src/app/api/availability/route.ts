...
export async function GET(request: NextRequest) {
  ...
  const { startUtc, endUtc } = dayRangeUtc(date, business.timezone);

  const [{ data: service, error: serviceError }, { data: hours, error: hoursError }, { data: existing, error: apptError }] =
    await Promise.all([
      supabaseServer.from("services").select("*").eq("id", serviceId).single(),
