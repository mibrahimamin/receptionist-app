...
/** Defense in depth: middleware already gates /dashboard, but actions can
 * in principle be invoked directly, so re-check the session here too. */
      ...
      cookies: {
        ...
        setAll(cookiesToSet) {
          try {
            ...
            // Middleware refreshes authentication cookies.
          }
        },
      },
    }
...
export async function upsertFaq(formData: FormData) {
  ...
  if (id) {
    await supabaseServer
      .from("faqs")
      .update({ question, keywords, answer, sort_order: sortOrder })
      .eq("id", id)
      .eq("business_id", businessId);
  } else {
    await supabaseServer
      .from("faqs")
      .insert({ business_id: businessId, question, keywords, answer, sort_order: sortOrder });
  }
...
export async function deleteFaq(formData: FormData) {
  ...
  await supabaseServer.from("faqs").delete().eq("id", id).eq("business_id", businessId);
  revalidatePath("/dashboard/faqs");
}
...
export async function upsertService(formData: FormData) {
  ...
  // Refresh the main public Front Desk.
  revalidatePath("/");

  // Refresh all business slug Front Desk pages.
  revalidatePath("/front-desk/[slug]", "page");
}
...
export async function updateBusinessHour(formData: FormData) {
  ...
  await supabaseServer
    .from("business_hours")
    .update({ is_closed: isClosed, open_time: openTime, close_time: closeTime })
    .eq("business_id", businessId)
...
export async function updateBusinessSettings(formData: FormData) {
  ...
  if (!name || !greeting || !timezone) {
    throw new Error("Name, greeting, and timezone are required.");
  }

  await supabaseServer
    .from("businesses")
    .update({ name, tagline, greeting, timezone, slot_interval_minutes: slotInterval })
    .eq("id", businessId);
...
export async function updatePasscode(formData: FormData) {
  const businessId = await requireSession();
  const newPasscode = (formData.get("passcode") as string)?.trim();
  const confirmPasscode = (formData.get("confirm_passcode") as string)?.trim();
  ...
  await supabaseServer
    .from("businesses")
    .update({ passcode_hash: `${salt}:${hash}` })
    .eq("id", businessId);
